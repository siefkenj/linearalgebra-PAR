...
\begin{document}
	...
	\setheader{MAT224}{PAR 6}{Due: Week 10 Tutorial (draft), March 15 (final)}{}{Name:\underline{\hspace{2in}}}{}
	...
	A \emph{random vector} in $\R^n$ is a vector whose components (when written
	in the standard basis) are independently and uniformly chosen random numbers in the interval $[0,1]$.

	{\bf Random Vector Theorem}: Let $\mathcal V\subset \R^n$ be a subspace of $\R^n$ of dimension $m<n$
	and let $\vec x\in\R^n$ be a random vector. The probability that $\vec x\in\mathcal V$ is zero.

	\bigskip
	{\bf Problem Statement}\par
	
	Let $\vec x_1,\ldots,\vec x_n\in\R^n$ be $n$ random vectors.\footnote{If you are familiar with probability theory,
	$\vec x_1,\ldots,\vec x_n$ are independent random variables---which is a distinct concept from linear
	independence.}
	Prove that there is a 100\% chance that $\{\vec x_1,\ldots,\vec x_n\}$ 
	form a basis for $\R^n$.
	
	\emph{Hint: the random vector theorem is your friend. Use it!}
