...
\begin{document}
	...
	\setheader{MAT224}{PAR 4}{Due: Week 6 Tutorial (draft), February 15 (final)}{}{Name:\underline{\hspace{2in}}}{}
	...
	A linear transformation $\mathcal P:\mathcal V\to\mathcal V$ is called
	a \emph{projection} if $\mathcal P\circ \mathcal P=\mathcal P$ (you might
	be familar with \emph{orthogonal projections} which further require $\vec v-\mathcal P\vec v$
	to be orthogonal to $\mathcal P\vec v$).

	Let $\mathcal T:\mathcal V\to\mathcal V$ be a linear transformation and
	let $\mathcal V'\subseteq \mathcal V$ be a subspace. The \emph{restriction} of
	$\mathcal T$ to $\mathcal V'$, written $\mathcal T\Big|_{\mathcal V'}$, is the linear transformation
	\[
		\mathcal T\Big|_{\mathcal V'}:\mathcal V'\to\mathcal V
	\]
	so that $\mathcal T\Big|_{\mathcal V'}(\vec v)=\mathcal T(\vec v)$ for all $\vec v\in\mathcal V'$
	(and which is undefined for $\vec v\notin\mathcal V'$).

	\bigskip
	{\bf Problem Statement}\par

	Let $\mathcal P:\mathcal V\to\mathcal V$ be a projection. Show that $\mathcal P\Big|_{\text{range}(\mathcal P)}$
	is the identity map on $\text{range}(\mathcal P)$.
	
		
	\emph{Hint: if you find yourself using cumbersome notation over and over again, 
	name some new variables to aid your exposition.}
	\bigskip
