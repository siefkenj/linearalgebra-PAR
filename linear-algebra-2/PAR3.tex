...
\begin{document}
	...
	\setheader{MAT224}{PAR 3}{Due: Week 4 Tutorial (draft), February 1 (final)}{}{Name:\underline{\hspace{2in}}}{}
	...
	Let $\mathcal X$ be a real vector space.
	Suppose $\{\vec v_1, \vec v_2, \vec v_3\}\subset \mathcal X$ is a linearly independent set,
	and suppose $\{\vec w_1, \vec w_2, \vec w_3\}\subset \mathcal X$ is a linearly dependent set.
	Define $\mathcal V=\Span\{\vec v_1,\vec v_2,\vec v_3\}$ and $\mathcal W=\Span\{\vec w_1,\vec w_2,\vec w_3\}$.
	
	\begin{enumerate}
		
		\item[(a)] Is there a linear transformation $\mathcal{P}:\mathcal V\to\mathcal W$ such that $\mathcal{P}(\vec v_i) = \vec w_i$ for $i=1,2,3$?
		
		\item[(b)] Is there a linear transformation $\mathcal{Q}:\mathcal W\to\mathcal V$ such that $\mathcal{Q}(\vec w_i) = \vec v_i$ for $i=1,2,3$?
		
		
	\end{enumerate}
		
	\emph{Hint: the easiest way to show a linear transformation exists is to \emph{define} a particular linear transformation.
	To define a linear transformation, you must specify what it does to every element in its domain.}
	\bigskip
