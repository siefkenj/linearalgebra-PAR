...
\begin{document}
	...
	\setheader{Math 281-3}{PAR 4}{Due: 5/25 (draft), 5/26 (final)}{}{Name:\underline{\hspace{2in}}}{}
	...
	Let $\vec a,\vec b\in \R^3$ be linearly independent and let $\vec c=\vec a\times \vec b$ (the cross product of $\vec a$ and $\vec b$).
	Finally, let $M=[\vec a|\vec b|\vec c]$ be the matrix with columns $\vec a$, $\vec b$, and $\vec c$.
	
	\begin{enumerate}
		\item Could det$(M)$ ever be zero?
		\item Could det$(M)$ ever be negative?
		\item Explain why det$(M)=\|\vec c\|^2$.
	\end{enumerate}

	Try to address all of these questions in a single narrative that takes less than one page.
